...
| `generated/shardrun/map-chamber-<layer>.png`, `map-wall-<layer>-0.png`, `map-prop-{rest,forge,treasure}.png` | The layer map as a place (ADR-0021): one painted chamber per layer (128x96, img2img from the `chamber-<layer>` sketch in `scripts/art/layouts.py`, so all three share an arched shape), a seamless dark rock tile per layer (96x96) behind the map, and a campfire, an anvil and a chest (64x64 sprites) standing in rest, forge and treasure rooms. Picked with foes composited in, so lit foes and silhouettes both read against each chamber. The anvil is drawn with the anime checkpoint the foes use: the pixel-art base model draws an anvil as a T on a pedestal (compare `props/anvil`). | Every room of `LayerMap.tsx`; fights show their foes' sprites instead of a prop, and the guardian's room uses the layer's arena. Without them the chambers are drawn in CSS and the old 32x32 icons stand in. |
